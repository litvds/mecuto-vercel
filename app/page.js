  ...
  return (
    <main style={mainStyle}>
      <div style={heroWrapStyle}>
        ...
        <div style={heroSubStyle}>Подбор модели, опции и PDF в одном интерфейсе.</div>
      </div>
                    ...
                    {grouped[key].items.map((opt, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={optionHintStyle}>{disabled ? "Входит в комплектацию" : checked ? "Добавлено" : "Нажмите, чтобы добавить"}</div>
                        </button>
                      );
                    })}
                  </div>
...
function Block({ title, children }) {
  return <div style={blockStyle}><div style={blockTitleStyle}>{title}</div>{children}</div>;
}
...
function Row({ label, value }) {
  return <div style={rowStyle}><div style={{ color: "#666" }}>{label}</div><div style={{ fontWeight: 700 }}>{fmt(value)}</div></div>;
}
