...
function drawTable(page, startY, headers, rows, colWidths, font, bold, bodySize = 10, headSize = 10) {
  ...
  let x = x0;
  page.drawRectangle({
    x: x0, y: y - 22, width: tableWidth, height: 22,
    borderWidth: 1, borderColor: rgb(0.82, 0.82, 0.82)
  });

  headers.forEach((h, i) => {
    page.drawRectangle({
      x, y: y - 22, width: colWidths[i], height: 22,
      borderWidth: 1, borderColor: rgb(0.82, 0.82, 0.82)
    });
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    const heights = row.map((cell, i) => {
      const lines = wrapText(String(cell || ""), highlight ? bold : font, bodySize, colWidths[i] - 8);
    ...
    row.forEach((cell, i) => {
      page.drawRectangle({
        ...
        color: highlight ? LIGHT_GRAY : undefined
      });

      const useFont = highlight ? bold : font;
...
export async function buildPdfBuffer(data) {
  ...
  page.drawRectangle({
    x: M, y: y - 130, width: A4_W - 2 * M, height: 130,
    borderWidth: 1, borderColor: rgb(0.84, 0.84, 0.84)
  });
  ...
  drawWrapped(page, data.page2Text || "", M, y, {
